...
const buildCourseList = (levelFilter) => {
  ...
  for (const [path, module] of Object.entries(allJsonModules)) {
    ...
    if (isSprach) {
      if (fileName !== 'content.json') continue;
    } else {
      if (!fileName.match(/^q\d+\.json$/)) continue;
    }
...
const Lesen = () => {
  ...
  // ─── KEY FIX: Filter ONLY by subTab ────────────────────────────────────────
  const filteredCourses = useMemo(() => {
    const q = search.trim().toLowerCase();
    const tab = courses.filter(c => c.subTab === subTab);
    if (!q) return tab;
    return tab.filter(c =>
      c.titleDe.toLowerCase().includes(q) || c.titleAr.includes(q)
    );
  }, [courses, subTab, search]);

  const handleCardClick = (course) => {
    const routeBase = course.type === 'sprach' ? 'sprachbausteine' : 'lesen';
        ...
        <div className="px-4 md:px-8 lg:px-12 pb-16 max-w-7xl mx-auto">
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-400">
              <p className="text-sm mb-2">لا توجد مواضيع في {subTab}</p>
              <p className="text-xs text-gray-500">
                المجلدات المتاحة: {availableSubTabs.join(', ') || 'لا يوجد'}
              </p>
            </div>
          ) : (
