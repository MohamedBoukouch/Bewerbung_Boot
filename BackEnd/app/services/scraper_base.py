...
class BaseScraper:
    def __init__(self, profession: str, location: str = "", max_results: int = 50,
        ...
        self.seen_emails = set()     # Track emails to avoid duplicates
        self.seen_domains = set()    # Track website domains to avoid duplicates
        self.source_label = ""
    ...
    def _domain_from_url(self, website: str) -> str:
        """Extract the domain from a website URL for dedup."""
        if not website:
            return ""
        try:
    ...
    def _add_company(self, name: str, email: str, city: str, website: str, phone: str, job_title: str, latitude: float = None, longitude: float = None, formatted_address: str = ""):
        """Add company to results. ONLY if email is present.
        Smart dedup: skips duplicates by normalized name, email, or website domain."""
        ...
        # DEDUP: Skip if same company name already in CURRENT scrape
        norm_name = name.strip().lower()
        if norm_name in self.seen_companies:
            self.log("info", f"Skipping duplicate in current scrape: {name} ({city})")
            return
        self.seen_companies.add(norm_name)

        # DEDUP: Skip if same email already in CURRENT scrape
        norm_email = email.lower()
        if norm_email in self.seen_emails:
            self.log("info", f"Skipping duplicate email in current scrape: {email} ({name})")
            return
        self.seen_emails.add(norm_email)

        # DEDUP: Skip if same website domain already in CURRENT scrape
        domain = self._domain_from_url(website)
        if domain and domain in self.seen_domains:
            self.log("info", f"Skipping duplicate domain in current scrape: {domain} ({name})")
            return
        if domain:
